...
const categoryMeta: Record<CompanyDocument['category'], { id: string; title: string }> = {
  licencje: { id: 'licencje', title: 'Licencje i Certyfikaty' },
  polisy: { id: 'polisy', title: 'Polisy i Ubezpieczenia' },
};
...
async function getDocuments(): Promise<CompanyDocument[]> {
  return client.fetch(documentsQuery, {}, { next: { revalidate: 3600 } });
}

export default async function DocumentsList() {
  const docs = await getDocuments();

  const categories = (['licencje', 'polisy'] as CompanyDocument['category'][])
    .map(cat => ({
      ...categoryMeta[cat],
    ...
    >
      <div className="max-w-7xl mx-auto space-y-20">
        {categories.map(category => (
          <div key={category.id}>
            <div className="flex items-center gap-4 mb-10" id={`category-${category.id}`}>
              <span className="w-1.5 h-8 bg-secondary rounded-full shrink-0" aria-hidden="true" />
